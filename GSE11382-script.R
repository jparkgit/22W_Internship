...
dds_HSC_CMP <- DESeqDataSetFromMatrix(countData = HSC_CMP ,
                               colData = metadata_1,
...
dds_CMP_GMP <- DESeqDataSetFromMatrix(countData = CMP_GMP ,
                               colData = metadata_2,
...
dds_CMP_MEP <- DESeqDataSetFromMatrix(countData = CMP_MEP ,
                               colData = metadata_3,
...
dds <- DESeqDataSetFromMatrix(countData = GSE11382_pcg,
                               colData = metadata,
...
res %>% as.data.frame %>% filter(abs(log2FoldChange) > 1, padj < 0.05) %>% dim
res %>% as.data.frame %>% filter(abs(log2FoldChange) > 1, padj < 0.05) %>% head
res %>% as.data.frame %>% filter(abs(log2FoldChange) > 2, padj < 0.05) %>% head
